...
if(this.state.markResult==""){
  ...
  this.setState({isLoader:false})
    ...
    formData.append("prefront_image",  {
      uri: this.state.markResult,
      type: 'image/jpeg',
      name: 'image.jpg',
    });
    axios.post(`https://www.brandspring.in/BSIS/api/save-prefront-image/${userid}`,   
     formData, {
      headers: {
...
.then((res)=> {
  console.log('data......',res.data)
    ...
    this.props.navigation.navigate('PostFrontPcture',{shop_id:res.data.shop_id})
    }
    else {
...
<Text style={{fontSize:18}}>Click PRe Front Picture</Text>
  </View>
